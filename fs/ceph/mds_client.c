			   ...
			   struct ceph_msg *msg)
{
	...
	if (msg_version >= 6) {
		...
		for (i = 0; i < cap_auths_num; i++) {
			...
			for (j = 0; j < _len; j++)
				ceph_decode_64_safe(&p, end, cap_auths[i].match.gids[j], bad);

			ceph_decode_32_safe(&p, end, _len, bad);
			cap_auths[i].match.path = kcalloc(_len, sizeof(char), GFP_KERNEL);
			...
			ceph_decode_32_safe(&p, end, _len, bad);
			cap_auths[i].match.fs_name = kcalloc(_len, sizeof(char), GFP_KERNEL);
